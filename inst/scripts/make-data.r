...
# the model to protect biological effects as were not used to adjust the original data.

### The metabric for breast cancer data (https://www.nature.com/articles/nature10983)
# was also downloaded but no extra data processing was undertaken. Each of the expression
# matrices from GEO were standardized gene-wise using the metabric dataset as a
# reference.

### Survival information (relapse event and months to relapse) was annonated
# as part of the KM plotter version 2010 (https://www.ncbi.nlm.nih.gov/pubmed/20020197)
...
# For doing so, a linear model was fitted separately for each probeset that included
# these metrics as the only explanatory variables, and coefficients of such models were
# used to correct the expression values a-priori.

### A second linear model was fitted to each probeset and dataset separately,
# in order to correct by potential technical effects captured by sample’s center
# of origin and batch (scanning day). This correction was carried out using a
# mixed-effect model in which gender, age at diagnosis, stage, tumour location and
# Microsatellite Instability (MSI) status were also included as covariates, when available.
# Scanning day was modeled as a random effect in these models, while center was included as a
# fixed (GSE14333) or a random effect (GSE39582 and GSE37892) depending on the number of
# centers involved and on the sample size in each of them.

###  GSE14333, GSE37892 and GSE33113 expression matrices were standardized gene-wise
# using the GSE39582 dataset as a reference.
