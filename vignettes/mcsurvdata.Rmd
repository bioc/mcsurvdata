...
to identify abnormal samples. Technical information concerning samples
...
the information available in Affymetrix. No samples were excluded due
to quality issues.
...
covariates, when available. Scanning day was modelled as a random
...
from GEO were standardised gene-wise using the metabric dataset as a
...
data. Eklund metrics are also computed in atttibutes `pm.med`,
`pm.iqr`,   `rma.iqr` and    `rna.deg`. Unique charactaristics of the
tumors such as `stage`, `msi` information and `cms` are annotated in the
colon cancer cohorts, whereas `ER.status`, `PGR.status` and
`HER2.status` are annoted in the breast cancer cohorts.
